...
/*
 * Displays a session and handles its joining logic.
 * Gets session and enforces access (year group, cancellation rules).
 * Determines state (joined, full, upcoming/ongoing/expired).
 * Allows users to join if eligible (not joined, not full, not cancelled, not expired).
 * Shows session details, status tags, and appropriate messages.
 */
...
$canJoin =
    !$joined &&
    !$full &&
    !session_row_is_cancelled($session) &&
    in_array($availability, ['upcoming', 'ongoing'], true);

$message = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['join'])) {
    if (!in_array($availability, ['upcoming', 'ongoing'], true)) {
...
<?php elseif ($joined): ?>
    <p class="msg msg-ok">You are signed up for this session. You can manage it anytime from the "My Sessions" page.  </p>
